...
const BLEScreen: React.FC<BLEScreenProps> = ({ navigation }) => {
  ...
  /**
   * Handle Direct Connection - try connecting directly using MAC address (Android API 31+)
   */
  const handleDirectConnection = async () => {
    if (Platform.OS === 'android' && Platform.Version >= 31) {
      ...
      if (connected) {
        Alert.alert('Success', 'Direct connection successful!');
      } else {
        Alert.alert('Failed', 'Direct connection failed. Try scanning instead.');
      }
    } else {
      Alert.alert('Not Available', 'Direct connection is only available on Android API 31+');
    }
  };
      ...
      <View style={[
        ...
        {!isConnected && !targetDevice && !isSearching ? (
          ...
          <>
                  ...
                  ]} onPress={() => navigation.navigate('BLESearch')}>
                    <Text style={[
                      styles.actionButtonText,
                      { fontSize: isSmallScreen ? 16 : 18 }
                    ]}>Manual Search</Text>
                  ...
                  {/* Direct Connect button for Android API 31+ */}
                  {Platform.OS === 'android' && Platform.Version >= 31 && (
                    ...
                    ]} onPress={handleDirectConnection}>
                      <Text style={[
                        styles.actionButtonText,
                        { fontSize: isSmallScreen ? 16 : 18 }
                      ]}>Direct Connect</Text>
